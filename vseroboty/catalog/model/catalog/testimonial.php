...
class ModelCatalogTestimonial extends Model {
	...
	public function addTestimonial($data, $status) {
		...
		$testimonial_id = $this->db->getLastId(); 
			...
			$this->db->query("INSERT INTO " . DB_PREFIX . "testimonial_description SET testimonial_id = '" . (int)$testimonial_id . "', language_id = '".(int)$this->config->get('config_language_id')."', title = '" . $this->db->escape($data['title']) . "', description = '" . $this->db->escape(strip_tags($data['description'],'<p><b><i>')) . "'");
